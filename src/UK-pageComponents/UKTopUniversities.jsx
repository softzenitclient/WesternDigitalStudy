...
import { MapPin, Award, BookOpen, ArrowUpRight } from 'lucide-react';
import { motion } from 'motion/react';
import { useSheetData } from '../context/SheetDataContext';

export default function UKTopUniversities({ onApplyNowClick }) {
  const { detailedUniversities: DETAILED_UNIVERSITIES } = useSheetData();
  const topUnis = DETAILED_UNIVERSITIES.filter(
    (uni) => uni.country.toLowerCase() === 'uk' || uni.country.toLowerCase() === 'united kingdom'
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="space-y-1">
          ...
          <p className="text-gray-500 text-sm max-w-xl">
            Study inside world-famous academic landmarks featuring phenomenal research databases and immediate Post-Study graduation work pathways.
          </p>
        </div>
      </div>
