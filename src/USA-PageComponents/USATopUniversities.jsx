...
export default function USATopUniversities({ onApplyNowClick }) {
  const { detailedUniversities: DETAILED_UNIVERSITIES } = useSheetData();
  const topUnis = DETAILED_UNIVERSITIES.filter(
    (uni) => uni.country.toLowerCase() === 'usa' || uni.country.toLowerCase() === 'united states'
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="space-y-1">
          ...
          <p className="text-gray-500 text-sm max-w-xl">
            Join Ivy Leagues or leading state research complexes equipped with dynamic STEM OPT work authorizations.
          </p>
        </div>
      </div>
