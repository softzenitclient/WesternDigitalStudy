...
export default function ApplicationSection({ onApplyClick }) {
  ...
  const defaultHero = {
    ...
    description: "At Western Study, we simplify the university application process through personalized guidance and expert support, making every step seamless, transparent, and stress-free. Our dedicated counselors work closely with each student to ensure applications are strategically prepared, maximizing opportunities for admission to leading institutions worldwide.",
  ...
  const sectionData = servicesDetailedSections?.find(s => s.sectionKey === 'application') || defaultHero;
  ...
  const renderTitle = () => {
    ...
    if (sectionData.title.toLowerCase().includes('university application')) {
      return <>University <br className="hidden md:block" /> Application</>;
    }
