...
import { ChevronLeft, ChevronRight, ArrowRight, MousePointerClick } from 'lucide-react';

export default function HeroSlider({ onApplyNowClick }) {
  const { slides: SLIDES } = useSheetData();
  ...
  useEffect(() => {
    ...
    timeoutRef.current = setTimeout(
      ...
      6000 // 6 seconds auto rotatation
    );
    return () => {
      resetTimeout();
    };
  ...
  return (
    <div id="hero-slider-section" className="relative h-[80vh] md:h-[90vh] w-full overflow-hidden bg-gray-900 mt-[100px]">
                ...
                <div className="max-w-4xl mx-auto w-full text-white">
                  <div className="space-y-6">
                    ...
                    <motion.div 
                      ...
                      className="inline-flex items-center gap-2 bg-[#f15b24]/20 border border-[#f15b24]/40 px-4 py-1.5 rounded-full text-[#f15b24] font-semibold text-xs md:text-sm uppercase tracking-wider"
                    >
      ...
      <button
        ...
        onClick={prevSlide}
        className="absolute left-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-[#f15b24] hover:scale-110 text-white p-3 rounded-full transition-all cursor-pointer hidden sm:flex border border-white/10"
      >
        <ChevronLeft size={24} />
      </button>
      <button
        ...
        onClick={nextSlide}
        className="absolute right-4 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-[#f15b24] hover:scale-110 text-white p-3 rounded-full transition-all cursor-pointer hidden sm:flex border border-white/10"
      >
        ...
        {SLIDES.map((_, index) => (
          <button
            ...
            className={`h-2.5 rounded-full transition-all duration-300 ${
              index === currentIndex ? 'w-8 bg-[#f15b24]' : 'w-2.5 bg-white/40 hover:bg-white/70'
            }`}
          />
        ))}
      </div>
